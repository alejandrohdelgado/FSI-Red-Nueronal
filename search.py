...
class PriorityQueue(utils.Queue):
    """Cola que ordena elementos por una función de coste (f).
    Necesaria para Ramificación y Acotación."""
    def __init__(self, f):
        self.A = []
        self.f = f
    ...
    def append(self, item):
        self.A.append(item)
        # Ordenamos la cola cada vez que insertamos (ineficiente para grandes datos
        # pero suficiente para este grafo y cumple con la interfaz).
        self.A.sort(key=self.f)
...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            ...
            print(f"Costo total: {node.path_cost}")
            print(f"Tiempo: {end_time - start_time:.6f} s")
            print(f"Ruta: {node.path()}")
            return node
        if node.state not in closed:
            closed[node.state] = True
            hijos = node.expand(problem)
            fringe.extend(hijos)
            generados += len(hijos)
...
def branch_and_bound_graph_search(problem):
    ...
    return graph_search(problem, PriorityQueue(lambda n: n.path_cost))

def branch_and_bound_with_heuristic_graph_search(problem):
    """Ramificación y acotación con subestimación (A*). [cite: 17, 18]"""
    return graph_search(problem, PriorityQueue(lambda n: n.path_cost + problem.h(n)))
